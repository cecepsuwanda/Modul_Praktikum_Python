...
\section{Contoh tambahan: \texttt{input} angka dengan penanganan sederhana}

\noindent Karena \texttt{input()} mengembalikan \texttt{str}, konversi ke \texttt{int} dapat gagal jika pengguna mengetik bukan angka. Pola \texttt{try}/\texttt{except} minimal berikut mempersiapkan modul berkas nanti; lihat tutorial pengecualian (\href{https://docs.python.org/3/tutorial/errors.html}{Errors and Exceptions}, lampiran \textbf{[8]}).

\begin{lstlisting}[caption=Konversi input dengan try/except ValueError]
try:
    x = int(input("Masukkan bilangan bulat: "))
    print("Berhasil:", x)
except ValueError:
    print("Bukan bilangan bulat yang valid.")
\end{lstlisting}
