...
\section{Contoh tambahan: \texttt{enumerate} dengan indeks mulai bukan nol}

\noindent Fungsi bawaan \texttt{enumerate} berguna saat Anda perlu indeks sekaligus elemen, misalnya untuk menampilkan tabel bernomor. Argumen \texttt{start} menggeser basis indeks tanpa mengubah koleksi asli; lihat bagian loop pada \href{https://docs.python.org/3/tutorial/controlflow.html}{tutorial kontrol alur} (lampiran \textbf{[8]}).

\begin{lstlisting}[caption=enumerate dengan start=1]
nama = ["Ani", "Budi", "Citra"]
...
\noindent Varian yang sama untuk \texttt{while}: jika loop selesai tanpa pernah mengeksekusi \texttt{break}, blok \texttt{else} dianggap jalur ``tidak ditemukan''. Cuplikan berikut mencari nilai dalam daftar; bandingkan dengan pola serupa di \href{https://docs.python.org/3/tutorial/controlflow.html}{bab kontrol alur tutorial resmi} (lampiran \textbf{[8]}).
