...
\section{Contoh tambahan: operator walrus (\texttt{:=})}

\noindent Python 3.8+ memperkenalkan operator penugasan ekspresi (\textit{walrus}) yang memadukan penilaian dan penugasan; berguna saat Anda ingin menyimpan hasil sambil menguji kondisi. Ringkasan sintaks pada \href{https://docs.python.org/3/whatsnew/3.8.html}{What's New 3.8} dan tutorial kontrol alur (lampiran \textbf{[8]}).

\begin{lstlisting}[caption=Walrus pada kondisi while]
n = 0
while (baris := input("Ketik 'selesai' untuk berhenti: ")) != "selesai":
    n += 1
    print(f"Baris ke-{n}: {baris!r}")
\end{lstlisting}
