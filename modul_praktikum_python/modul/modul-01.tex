...
\begin{table}[htbp]
\centering
\begin{tabular}{|l|>{\raggedright\arraybackslash}p{4.2cm}|>{\raggedright\arraybackslash}p{5.8cm}|}
\hline
\textbf{Mode} & \textbf{Kapan memakai} & \textbf{Rujukan belajar terbuka} \\
\hline
REPL & Eksperimen cepat, kalkulator, cek sintaks & Tutorial Python bab 3; \textbf{[8]}, \textbf{[9]} \\
\hline
Skrip \texttt{.py} & Program utuh, tugas, otomasi & Tutorial bab 2--4; \textbf{[12]} \\
\hline
Jupyter & Eksplorasi data, laporan campur teks/kode & Modul UIMA \textbf{[6]}; \textbf{[4]} (repositori contoh) \\
\hline
\end{tabular}
\caption[Perbandingan mode menjalankan Python]{Perbandingan mode menjalankan Python; tautan tutorial resmi di \texttt{sumber\_materi.md}.}
\label{tab:mode-jalankan}
\end{table}
...
\subsection{Mengetahui versi interpreter}

\noindent Menyamakan versi interpreter dengan panduan laboratorium mengurangi perbedaan perilaku antar mesin. Modul pustaka \texttt{sys} menyediakan informasi versi yang dapat dicetak dari skrip; dokumentasi resmi: \href{https://docs.python.org/3/library/sys.html}{\texttt{sys}} pada indeks pustaka standar (lampiran \textbf{[20]}), konteks instalasi pada \textit{The Python Tutorial} bab 2 (lampiran \textbf{[8]}, \textbf{[9]}).

\begin{lstlisting}[caption=Cek versi Python dari skrip]
import sys
