...
\centering
\small
\begin{tabular}{|l|c|c|>{\raggedright\arraybackslash}p{4.5cm}|}
\hline
\textbf{Tipe} & \textbf{Mutable} & \textbf{Terurut*} & \textbf{Kegunaan ringkas} \\
\hline
\texttt{list} & Ya & Ya & Koleksi berubah, stack/queue sederhana \\
\hline
\texttt{tuple} & Tidak & Ya & Rekaman tetap, kunci \texttt{dict} \\
\hline
\texttt{dict} & Ya & Ya (3.7$+$)\dag & Pemetaan kunci $\rightarrow$ nilai \\
\hline
\texttt{set} & Ya & Tidak & Unik, operasi himpunan \\
\hline
\end{tabular}
\caption{Ringkasan struktur data inti; rinci di \textit{The Python Tutorial} bab 5: \protect\url{https://docs.python.org/3/tutorial/datastructures.html} --- \textbf{[8]}, \textbf{[9]}; panduan struktur data \textbf{[19]}. \textit{*Urutan iterasi. \dag Perilaku urutan kunci \texttt{dict} pada Python 3.7+.}}
...
\section{Contoh tambahan: \texttt{collections.Counter}}

\noindent \texttt{Counter} menghitung frekuensi elemen pada iterable atau pemetaan; cocok untuk ringkasan data sederhana sebelum memasuki \texttt{pandas}. Dokumentasi modul \href{https://docs.python.org/3/library/collections.html#collections.Counter}{\texttt{collections}} termasuk indeks pustaka standar (lampiran \textbf{[20]}).

\begin{lstlisting}[caption=Counter pada teks token sederhana]
from collections import Counter

teks = "python python praktikum"
hitung = Counter(teks.split())
print(hitung)          # Counter({'python': 2, 'praktikum': 1})
print(hitung.most_common(1))
\end{lstlisting}
